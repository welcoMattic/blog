...
> **Source code.** The [`03-frankenphp`](https://github.com/welcoMattic/symfony-clever-cloud-series/tree/03-frankenphp) branch of the [welcoMattic/symfony-clever-cloud-series](https://github.com/welcoMattic/symfony-clever-cloud-series) repository contains what this article adds: the root `php.ini`, which is added next to `public/.user.ini` rather than replacing it, the `clevercloud/post-build-frankenphp.sh` hook and its `importmap:install`, the Redis session configuration in `config/packages/framework.yaml`, the second entry in `.clever.json`, and an example `Caddyfile` left inactive.
